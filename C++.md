...
## 深、浅拷贝 个人理解  
* 针对class 里面有pointer 成员的情况  
* 针对拷贝构造 和 拷贝复制的情况  
* 当拷贝一个指针成员时，这个指针成员 m_data ：
>默认拷贝构造(浅拷贝)：仅仅复制 m_data 这个指针变量，而不是这个指针变量所指向的(堆)内存空间   
造成问题：两个类的 m_data 指向同一块(堆)内存，如果lfs这个对象原来存在 m_data 且有(堆)内存，那这块内存将会泄露 
>深拷贝： 首先判断是否自己给自己赋值，然后讲lfs原有的内存释放 delete[] ,然后 new 一块新的内存，大小==存放rhs指针所指向的内存大小，然后一个一个拷贝过去。
## 深拷贝和浅拷贝
...
### 仿函数  
```md
使用场景：比如cont_if(it.begin(),it.end(),比较函数的函数指针) ==它要求这个比较函数只能有一个参数，当我们想该比较函数能传多个参数时，就不得行。==>使用仿函数，也就是一个封装成类的重载operator()的东西。因为封装成类，所以可以访问成员变量，构造函数只需要传递一个参数，并且他的对象会自动调用这个 operator()函数。
```
- 算法：包括查找算法、排序算法等等。
