...
_day8reak:在这个结构体中，char类型通常占用一个字节，int类型通常占用四个字节。然而，结构体中的最后一个成员是一个长度为0的数组，通常称为flexible array member（柔性数组成员）。这种数组的大小在实例化结构体时才被确定。因此，结构体的大小通常为成员中最大类型的大小的倍数，加上柔性数组成员的大小。因此，8字节中包含了char类型的1字节、int类型的4字节以及柔性数组成员的3字节对齐空间。
//sizeof = 4
struct str{
    int b[0];
}
_day8reak:这个结构体只有一个柔性数组成员，没有其他成员。柔性数组成员的大小在实例化结构体时确定，但没有其他成员，所以这个结构体的大小仅仅是柔性数组成员的大小。在这种情况下，编译器通常会对柔性数组成员进行特殊处理，使得该结构体的大小等于柔性数组成员的大小。
//sizeof = 1
struct str{
    
}
_day8reak:这个结构体没有任何成员，因此它的大小为0字节。但是C标准规定结构体的大小不能为0字节，因此编译器通常会将其大小设置为1字节，以确保每个结构体的实例都有一个非零大小。
```
...
_day8reak:示例：int (*ptr)[5]; 这里的 ptr 是一个指针，指向包含5个整数的数组。

指针的数组：一个数组的每一个元素都是指针

_day8reak:示例：int *ptr[5]; 这里的 ptr 是一个包含5个指针的数组，每个指针可以指向一个整数。
...
在C++中，深拷贝（Deep Copy）和浅拷贝（Shallow Copy）是两种不同的对象复制方式，它们之间的区别主要体现在拷贝对象的成员数据上。

浅拷贝（Shallow Copy）：

-浅拷贝只是简单地复制对象的成员变量的值到新对象中。如果对象中包含指针类型的成员变量，浅拷贝只会复制指针的值，而不会复制指针所指向的数据。
因此，如果源对象和目标对象都指向同一块内存地址，那么一个对象的修改会影响到另一个对象，因为它们共享相同的资源。

深拷贝（Deep Copy）：

-深拷贝会复制对象的所有成员数据，包括指针类型的成员变量所指向的数据。它会为目标对象分配新的内存空间，并将源对象的数据复制到新的内存空间中。
因此，即使源对象和目标对象具有相同的值，它们也各自拥有独立的内存空间，修改一个对象不会影响另一个对象。
